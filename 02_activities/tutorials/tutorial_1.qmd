...
Please download the three files from the following Google Drive link: <https://drive.google.com/drive/folders/1v-FbDqu95wdUje3foFXMdXrBjYEAJTB2?usp=drive_link>
...
```{bash}
head ./02_activities/tutorials/gwa.qc.fam
```
...
head ./02_activities/tutorials/gwa.qc.bim
```
...
plink2 --bfile ./02_activities/tutorials/gwa.qc --recode A --out ./02_activities/tutorials/gwa_qc_additive
```
...
geno <- fread("./02_activities/tutorials/gwa_qc_additive.raw")  # columns include FID, IID, and SNP genotype
geno[1:5,1:10]
```
...
```{bash,results='hide',message=FALSE, warning=FALSE}
plink2 --bfile ./02_activities/tutorials/gwa.qc --hardy --out ./02_activities/tutorials/gwa_qc_hwe
```
...
# View HWE results
hwe <- fread("./02_activities/tutorials/gwa_qc_hwe.hardy")
head(hwe)
```
...
```{bash,results='hide',message=FALSE, warning=FALSE}
plink2 --bfile ./02_activities/tutorials/gwa.qc --freq --out ./02_activities/tutorials/gwa_qc_freq
```

View frequency table: gwa_qc_freq.afreq includes columns for SNP, chromosome, allele counts, MAF.

```{r,message=FALSE, warning=FALSE}
freq <- fread("./02_activities/tutorials/gwa_qc_freq.afreq")
head(freq)
```
...
printf "%s\n" rs1525873 rs2902663 rs343376 rs3813199 > ./02_activities/tutorials/snplist.txt
...
```{bash, results='hide',message=FALSE, warning=FALSE}
plink2 --bfile ./02_activities/tutorials/gwa.qc \
      --extract ./02_activities/tutorials/snplist.txt \
      --recode A \
      --out ./02_activities/tutorials/geno.additive
# --keep samplist.txt can be used to extract a specific list of samples
```

```{r}
# Load genotype data
geno_subset <- fread("./02_activities/tutorials/geno.additive.raw")
...
```{r,message=FALSE, warning=FALSE}
set.seed(1234)
fam_all <- fread("./02_activities/tutorials/gwa.qc.fam")
N=dim(fam_all)[1];N
bim_all<- fread("./02_activities/tutorials/gwa.qc.bim")
M <- nrow(bim_all);M

indx.SNP=which((bim_all$V2)%in%c('rs1525873','rs2902663','rs343376','rs3813199'))

geno_mat<- readPlinkToMatrixByIndex('./02_activities/tutorials/gwa.qc', 1:N, indx.SNP)
