...
```r
```{r}
bim <- fread("./02_activities/data/gwa.qc.A1.bim", header = FALSE)
nrow(bim)
```
...
# Your code here...
```{r}
geno_subset <- fread("./02_activities/data/gwa_qc_A1_subset.raw")
geno_subset <- na.omit(geno_subset)
head(geno_subset)
```
```In additive coding:
- 0, 1, 2 counts the number of **A2/reference** alleles in the `.raw` file
...
af <- sapply(snps, function(snp) {
  x <- geno_subset[[grep(paste0("^", snp, "_"), names(geno_subset), value = TRUE)]]
...
```r
```{r}
hwe[ID %in% c("rs1861", "rs3813199", "rs3128342", "rs11804831"),
    .(ID, P)]
```
...
```{r}
names(geno_subset)
grep("^rs1861_", names(geno_subset), value = TRUE)
```
```r
```{r}
rs1861_name <- grep("^rs1861_", names(geno_subset), value = TRUE)
geno_subset$rs1861_geno <- geno_subset[[rs1861_name]]
head(geno_subset$rs1861_geno)
```
```{r}
table(geno_subset$PHENOTYPE)
summary(geno_subset$PHENOTYPE)
```
```r
```{r}
model_add <- lm(PHENOTYPE ~ rs1861_geno, data = geno_subset)
summary(model_add)
```
```r
```{r}
summary(model_add)$coefficients["rs1861_geno", "Pr(>|t|)"]
```
...
```         
# Your answer here...
```The beta coefficient represents the expected change in phenotype for each 1-unit increase in additive genotype coding at rs1861, holding all else constant.
```
...
```r
```{r}
p_add <- ggplot(geno_subset, aes(x = rs1861_geno, y = PHENOTYPE)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  theme_minimal() +
  labs(
    title = "Phenotype vs additive-coded rs1861",
    x = "Additive genotype",
    y = "Phenotype"
  )
...
```r
```{r}
geno_subset$rs1861_rec <- ifelse(geno_subset$rs1861_geno == 2, 1, 0)
table(geno_subset$rs1861_rec)
```
...
```r
```{r}
model_rec <- lm(PHENOTYPE ~ rs1861_rec, data = geno_subset)
summary(model_rec)
summary(model_rec)$coefficients["rs1861_rec", "Pr(>|t|)"]
```
...
```r
```{r}
p_rec <- ggplot(geno_subset, aes(x = rs1861_rec, y = PHENOTYPE)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  theme_minimal() +
  labs(
    title = "Phenotype vs recessive-coded rs1861",
    x = "Recessive-coded genotype",
    y = "Phenotype"
  )
...
```
The better-fitting model is the one with the lower AIC and/or higher adjusted R-squared. So model_add better fit
